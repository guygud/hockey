// ============================================================================
// ЗАЕЗДЫ
// ----------------------------------------------------------------------------
// Заезд — это тема кадра плюс набор модификаторов баланса. Он меняет ощущение
// попытки, но не переписывает правила: каждый mod это множитель к числу из
// balance.js.
//
//   gapMul   — зазор между клюшками (умеет только поджимать, см. gapFor)
//   winMul   — ширина окна ответа
//   foeMul   — доля чужих клюшек
//   crossMul — доля лобовых пар
//   drainMul — расход инерции
// ============================================================================

import { ICE_MARKS } from "./tuning.js";

/** Нейтральный заезд: обучение и разминка, никаких модификаторов. */
export const ROUND_NEUTRAL = {
  id: "clean",
  name: "РАЗМИНКА",
  bpm: 0,
  emojis: ICE_MARKS.emojis,
  theme: {
    shell: "#14081f",
    strip: ["#12081f", "#2a1050", "#6b3aa8"],
    floor: ["#5a2d8a", "#7a45b0", "#3d1866"],
    tint: [0.42, 0.35],
    lane: ["rgba(255,210,255,0.07)", "rgba(255,220,120,0.55)"],
    lines: "235,215,255",
  },
  mods: {},
};

export const ROUNDS = [
  {
    id: "night",
    name: "НОЧНАЯ СМЕНА",
    bpm: 118,
    emojis: ["❄️", "🌙", "🧊", "⭐"],
    theme: {
      shell: "#0a1020",
      strip: ["#080c18", "#122040", "#1a3a6a"],
      floor: ["#1a2a50", "#243868", "#0e1830"],
      tint: [0.5, 0.22],
      lane: ["rgba(140,190,255,0.07)", "rgba(160,210,255,0.5)"],
      lines: "180,210,255",
    },
    // Просторнее и мягче: дышащий заезд между двумя злыми.
    mods: { gapMul: 1.1, winMul: 1.1, foeMul: 0.9 },
  },
  {
    id: "blitz",
    name: "БЛИЦ",
    bpm: 140,
    emojis: ["⚡", "🎯", "🏒"],
    theme: {
      shell: "#061818",
      strip: ["#041414", "#0a3030", "#147070"],
      floor: ["#0e3a3a", "#1a5858", "#082828"],
      tint: [0.38, 0.4],
      lane: ["rgba(80,255,230,0.08)", "rgba(90,255,220,0.55)"],
      lines: "140,255,240",
    },
    // Плотно, но окно шире — темп, а не точность.
    mods: { gapMul: 0.82, winMul: 1.15 },
  },
  {
    id: "forge",
    name: "ПЛАВКА",
    bpm: 128,
    emojis: ["🔥", "🏆", "💥"],
    theme: {
      shell: "#1a0808",
      strip: ["#180606", "#4a1810", "#8a3020"],
      floor: ["#5a2018", "#7a3020", "#2a0c0c"],
      tint: [0.36, 0.42],
      lane: ["rgba(255,180,120,0.08)", "rgba(255,160,80,0.55)"],
      lines: "255,200,150",
    },
    // Самый злой: больше чужих, больше прыжков, инерция тает быстрее.
    mods: { foeMul: 1.25, drainMul: 1.1, crossMul: 1.3 },
  },
  {
    id: "long",
    name: "ДОЛГИЙ ПУТЬ",
    bpm: 112,
    emojis: ["💎", "👑", "✨"],
    theme: {
      shell: "#0c0818",
      strip: ["#0a0616", "#1a1040", "#3a2080"],
      floor: ["#241850", "#382870", "#140c30"],
      tint: [0.46, 0.28],
      lane: ["rgba(180,160,255,0.07)", "rgba(200,180,255,0.5)"],
      lines: "200,190,255",
    },
    mods: { drainMul: 0.9 },
  },
];
